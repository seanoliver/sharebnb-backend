...
app.post('/upload-image', upload.single('image'), async (req, res) => {
  try {
    ...
    const getObjectCommand = new PutObjectCommand({ Bucket: bucketName, Key: key });
    ...
    res.status(500).send({ message: 'An error occurred while uploading the image', error: error.message });
  }
});
